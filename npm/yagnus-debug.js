...
/* This file is released under the Free Yagn.us Open License */
...
org.yagnus.LevelSetUpYo = function ( ) {}
org.yagnus.LevelSetUpYo.prototype.initLevelSetUpYo = function ( ) {
   this.levels = [ ];
}
org.yagnus.LevelSetUpYo.prototype.updateLevel = function ( level ) {}
org.yagnus.LevelSetUpYo.prototype.addAt = function ( level , result ) {
   if (level in this.levels)
      this.levels [ level ].push ( result );
   else
      this.levels [ level ] = [ result ];
   this.updateLevel ( level );
}
org.yagnus.LevelSetUpYo.prototype.addAll = function ( otherLS ) {
...
// Users may wish to replace util class with underscore.js or one of the other projects having
// identical names.
...
org.yagnus.utils.equalsIgnoreMember = function ( a , b , ignoreFromA , ignoreFromB , strictType ) {
   if (typeof (strictType) == 'boolean' && strictType && typeof (a) != typeof (b))
      return false; // duh
   if (a === b || a == b)
      return true;
   ...
   if (a instanceof Array || a instanceof Object) {
      if (typeof (ignoreFromA) == 'undefined' || ignoreFromA == null)
         ignoreFromA = {};
      if (typeof (ignoreFromB) == 'undefined' || ignoreFromB == null)
         ignoreFromB = {};
      // compare two objects
      var seen = {};
      for (x in a) {
         if (!(x in ignoreFromA) && a.hasOwnProperty ( x )) {
            if (!b.hasOwnProperty ( x ) || !org.yagnus.utils.equalsIgnoreMember ( a [ x ] , b [ x ] , ignoreFromA , ignoreFromB ))
               return false;
            seen [ x ] = true;
         }
      }
      for (x in b) {
         if (!(x in ignoreFromB) && !(x in seen) && b.hasOwnProperty ( x )) {
            if (!a.hasOwnProperty ( x ) || !org.yagnus.utils.equalsIgnoreMember ( a [ x ] , b [ x ] , ignoreFromA , ignoreFromB ))
               return false;
         }
      }
      return true;
...
// This is a link method to create a bit field supporting these methods. We have a default
// implementation but feel free to use a more efficient implementation
// get(i) //Gets ith bit
// set(i,value); //Sets ith bit to value, if value is undefined, sets ith bit to true
// update(o); //takes all bits of o and 'or' into current bitfield. Current field is extended in
// size if o is bigger.
org.yagnus.utils.createBitField = function ( size ) {
   return new org.yagnus.utils.BitField ( size );
}

/**
 * object is a method that makes an object having fields whose name come from the array 'a', the
 * value of these fields are either in the array 'b' correspondingly, or else, if b is a function
 * then the value of field in a is the result evaluation of method b on that name.
 */
...
oy.copyArray=oy.arrayCopy=org.yagnus.utils.copyArray
...
oy.deepCopy=oy.copyDeep=org.yagnus.utils.deepCopy;

org.yagnus.utils.shallowDuckType = function ( tp , obj ) {
   if (tp instanceof Array) {
      for ( var i = 0; i < tp.length; ++i)
         if (!obj.hasOwnProperty ( tp [ i ] ))
            return false;
      return true;
   } else if (typeof (tp) == 'object') {
      for ( var i in tp)
         if (tp.hasOwnProperty ( i ) && (!obj.hasOwnProperty ( i ) || typeof (tp [ i ]) != typeof (obj [ i ])))
            return false;
      return true;
   } else
      return false;
...
org.yagnus.utils.BitField = function ( p ) {
   ...
   if (typeof (p) == 'object')
      ...
      this.a = new Array ( );
      for ( var i = Math.ceil ( p / 32 ) - 1; i > -1; --i)
         this.a [ i ] = 0;
   }
}
org.yagnus.utils.BitFieldMethods = function ( ) {
   ...
   this.orEq = function ( o ) {
      if (o instanceof Object && o.a instanceof Array)
         for ( var i = o.a.length - 1; i > -1; --i) {
            if (i >= this.a.length)
               this.a [ i ] = 0;
            var oai = o.a [ i ];
            if (typeof (oai) != 'number')
               oai = 0;
            this.a [ i ] |= oai;
         }
      return this;
...
org.yagnus.maths.SIntClass = function ( ) {
   this.initLevelSetUpYo ( );
}
org.yagnus.maths.SIntClassPrototype = {
         updateLevel : function ( level ) {
            ...
            while (l.length > 1) {
               ...
               if (l [ 0 ] < 0) {
                  if (l [ 1 ] < 0) {
                     // 2 negatives, may underflow.
                     var fromUnderFlow = org.yagnus.smallInt - l [ 0 ];
                     if (fromUnderFlow > l [ 1 ]) { // will underflow
                        t = fromUnderFlow - l [ 1 ]; // calculate smallInt - l[0] - l[1];
                        t = -1 * t; // now turns it into l[0] + l[1] - smallInt;
                        this.addAt ( level + 1 , -1 ); // add smallInt to total count.
                     } else
                        t = l [ 0 ] + l [ 1 ];
                  } else {
                     // 1 negative one positive can never overflow or underflow
                     t = l [ 0 ] + l [ 1 ];
                  }
               } else {
                  // 2 positive
                  var toOverflow = org.yagnus.bigInt - l [ 0 ];
                  if (toOverflow < l [ 1 ]) { // Will overflow
                     t = toOverflow - l [ 1 ]; // bigInt - l[0] - l[1];
                     t = -1 * t; // l[0] + l[1] - bigInt;
                     this.addAt ( level + 1 , 1 );
                  } else
                     t = l [ 0 ] + l [ 1 ];
               }
               l = this.levels [ level ] = l.slice ( 1 );
               l [ 0 ] = t;
            }
         } ,
         inc : function ( x ) {
            this.addAt ( 0 , x );
            if (this.levels [ 0 ].length >= 5)
               this.updateLevel ( 0 );
         } ,
         update : function ( x ) {
            for (i in x.levels) {
               if (x.levels [ i ] instanceof Array) {
                  if (this.levels [ i ] == null)
                     this.levels [ i ] = org.yagnus.utils.deepCopy ( x.levels [ i ] );
                  else
                     this.levels [ i ] = this.levels [ i ].concat ( x.levels [ i ] );
               }
            }
         } ,
         calc : function ( ) {
            ...
            for (i = 0; i < this.levels.length; ++i) {
               if (this.levels [ i ].length > 0) {
                  p = Math.pow ( 2 , 53 * i );
                  for ( var y = 0; y < this.levels [ i ].length; ++y)
                     s += p * this.levels [ i ] [ y ];
               }
            }
            return s;
...
org.yagnus.maths.SIntClass.prototype = new org.yagnus.LevelSetUpYo ( );
org.yagnus.utils.extend ( org.yagnus.maths.SIntClass.prototype , org.yagnus.maths.SIntClassPrototype );

org.yagnus.maths.SInt = function ( v ) {
   if (arguments.length == 0) {
      return new org.yagnus.maths.SIntClass ( );
   } else if (typeof (v) == 'number') {
      var ret = new org.yagnus.maths.SIntClass ( );
      ret.addAt ( 0 , Math.floor ( v ) );
      return ret;
...
org.yagnus.stats.UnivariateBase = function ( ) {
   this.checkNumber = function ( n ) {
      return org.yagnus.stats.checkNumber ( n ) && !this.skip ( n );
   }
   this._inc = function ( ) {}
   this._dec = function ( x ) {
      return this._inc ( -x );
      return this;
   }
   this._inc_helper = function ( addData , args ) {
      ...
      for ( var i = 0; i < args.length; ++i) {
         ...
         if (ai instanceof Object) {
            ...
            else if (ai instanceof org.yagnus.stats.UniStats)
               try {
                  this._update ( ai );
               } catch (e) {
                  ++this.bad;
		...
		this.o=this.observe=this.inc=function(){return this._inc_helper(true, arguments);}
		this.u=this.unobserve=this.dec=function(){return this._inc_helper(false, arguments);}
...
org.yagnus.stats.UniStatsMethods = function ( ) {
   ...
   this._dec = function ( x ) {
      if (this.checkNumber ( x )) {
         this.min = this.max = parseFloat ( 'nan' );
         this._my_inc ( -1 , x );
         this.c -= 2;
      } else
         ++this.bad;
   }
   this._inc = function ( x ) {
      return this._my_inc ( 1 , x );
   }
   this._my_inc = function ( sign , x ) {
      try {
         ...
         if (this.checkNumber ( x )) {
            ...
            if (this.mmind) {
               if (this.mmbuffer > x) {
                  if (this.mmbuffer > this.max)
                     this.max = this.mmbuffer;
                  if (x < this.min)
                     this.min = x;
               } else {
                  if (x > this.max)
                     this.max = x;
                  if (this.mmbuffer < this.min)
                     this.min = this.mmbuffer;
               }
               this.mmind = false;
            } else {
               this.mmbuffer = x;
   ...
   this._update = function ( other ) {
      ...
      if (this.checkNumber ( other.c ))
         this.c += other.c;
      if (this.checkNumber ( other.s ))
         this.s += other.s;
      if (this.checkNumber ( other.ss ))
         this.ss += other.ss;
      if (this.checkNumber ( other.sss ))
         this.sss += other.sss;
      if (this.checkNumber ( other.ssss ))
         this.ssss += other.ssss;
      if (this.checkNumber ( other.min ) && this.min > other.min)
         this.min = other.min;
      if (this.checkNumber ( other.max ) && this.max < other.max)
         this.max = other.max;
      if (this.checkNumber ( other.bad ))
         this.bad += other.bad;
      return this;
   }
   ...
   this.calc = function ( ) {
      ...
      if (this.c > 0) {
         ...
         ret.average = m1;
         if (!this.checkNumber ( ret.average ))
            delete ret.average;
         ret.variance = m2;
         if (!this.checkNumber ( ret.variance ))
            delete ret.variance;
         ret.standardDeviation = Math.pow ( ret.variance , .5 );
         if (this.c > 2) {
            ret.skewness = Math.pow ( this.c * (this.c - 1) , .5 ) / (this.c - 2) * m3 / Math.pow ( m2 , 1.5 );
            if (!this.checkNumber ( ret.skewness ))
               delete ret.skewness;
         }
         if (m2 > 0) {
            ret.kurtosis = m4 / m2 / m2;
            if (!this.checkNumber ( ret.kurtosis ))
               delete ret.kurtosis;
            else
               ret.excess_kurtosis = ret.kurtosis - 3;
         }
         if (this.mmind) {
            if (this.mmbuffer < this.min)
               this.min = this.mmbuffer;
         ...
         if (!this.checkNumber ( ret.minimum ))
            delete ret.minimum;
         ret.maximum = this.max;
         if (!this.checkNumber ( ret.maximum ))
            delete ret.maximum;
      }
      return ret;
   }
...
/**
 * The discrete multivariate stats gatherer. Computes the full coocurrence matrix O(v^k) space where
 * k is the number of variables and v is number of levels.
 * 
 * 
...
org.yagnus.stats.DiscreteStatsMethods = function ( ) {
   ...
   my.o = my.observe = my.inc = function ( ) {
		if( arguments.length==2 && 
			 typeof(arguments[0])=='number' && 
			 (arguments[1] instanceof Array) ) 
          this._inc_helper(arguments[1],arguments[2]);
		else
          this._inc_helper ( 1 , arguments );
   }
   my.minc = function ( c ) {
      var my2 = this;
      return function ( ) {
         my2._inc_helper ( c , arguments );
      }
   };

   my.u = my.unobserve = my.dec = function ( ) {
      this._inc_helper ( -1 , arguments );
   }
   my.mdec = function ( c ) {
      var my2 = this;
   ...
   my._inc_helper = function ( cnt , args ) {
      var nargs = [ ];
      for ( var i = 0; i < args.length; ++i) {
         if (args [ i ] instanceof org.yagnus.stats.DiscreteStats)
            try {
               this.update ( args [ i ] );
            } catch (e) {
               ++this.bad;
      ...
      while (nargs != null && nargs.length == 1 && nargs [ 0 ] instanceof Array) {
         nargs = args [ 0 ];
      }
   ...
   my.getCount = function ( ) {
      // copy arguments into an array. mongo doesn't put slice() on arguments
      // object.
      if (arguments.length > this.nvar)
         return 0;// surely 0
      args = [ ];
      var x = 0;
   ...
   my.calc = function ( ) {
      return org.yagnus.stats.initDiscreteSummaryStatistics ( this ); // return
      // a
      // copy
   }
...
/**
 * The multivariate stats gatherer. Computes the covariance matrix and related statistics using
 * O(k^2) space where k is the number of variables.
 * 
 * 
 * @author hc.busy@yagn.us
 * 
...
org.yagnus.stats.histogram.BasicHistogram = function ( low , high , width ) {
   if (org.yagnus.utils.shallowDuckType ( [ 'low' , 'high' , 'width' , 'counts' , 'total' , 'tooHigh' , 'tooLow' , 'bad' ] , low )) {
      ...
      throw {
         message : "Bad parameter to BasicHistogram constructor."
      }
   }
}
...
oys.BHist=oy.BHist = org.yagnus.stats.histogram.initBasicHistogram;
	...
	org.yagnus.maths.SInt = function(v){
		if(typeof(org.yagnus._.isSIntReady)!='boolean' || !org.yagnus._.isSIntReady){
			org.yagnus.maths.SIntClass.prototype=new org.yagnus.LevelSetUpYo();
			org.yagnus.utils.extend(org.yagnus.maths.SIntClass.prototype,org.yagnus.maths.SIntClassPrototype);
			org.yagnus._.isSIntReady = true;
		}
		if(arguments.length==0){
			return new org.yagnus.maths.SIntClass();
		}else if(typeof(v)=='number'){
